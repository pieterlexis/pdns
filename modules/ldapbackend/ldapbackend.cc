...
bool LdapBackend::list( const DNSName& target, int domain_id, bool include_disabled )
{
  ...
  {
    L << Logger::Warning << d_myname << " Unable to get zone " << target << " from LDAP directory: " << lt.what() << endl;
  ...
  {
    L << Logger::Error << d_myname << " Unable to get zone " << target << " from LDAP directory: " << le.what() << endl;
    throw PDNSException( "LDAP server unreachable" );   // try to reconnect to another server
  }
  catch( std::exception &e )
  {
    L << Logger::Error << d_myname << " Caught STL exception for target " << target << ": " << e.what() << endl;
...
inline bool LdapBackend::list_simple( const DNSName& target, int domain_id )
{
  ...
  DLOG( L << Logger::Debug << d_myname << " Search = basedn: " << dn << ", filter: " << filter << endl );
  d_msgid = d_pldap->search( dn, LDAP_SCOPE_SUBTREE, filter, (const char**) ldap_attrany );
...
inline bool LdapBackend::list_strict( const DNSName& target, int domain_id )
{
  ...
  {
    L << Logger::Warning << d_myname << " Request for reverse zone AXFR, but this is not supported in strict mode" << endl;
...
void LdapBackend::lookup( const QType &qtype, const DNSName &qname, DNSPacket *dnspkt, int zoneid )
{
  try
  {
    ...
    if( d_qlog ) { L.log( "Query: '" + qname.toStringRootDot() + "|" + qtype.getName() + "'", Logger::Error ); }
  ...
  {
    L << Logger::Error << d_myname << " Caught STL exception for qname " << qname << ": " << e.what() << endl;
    throw DBException( "STL exception" );
  }
}
...
void LdapBackend::lookup_simple( const QType &qtype, const DNSName &qname, DNSPacket *dnspkt, int zoneid )
{
  ...
  DLOG( L << Logger::Debug << d_myname << " Search = basedn: " << getArg( "basedn" ) << ", filter: " << filter << ", qtype: " << qtype.getName() << endl );
...
void LdapBackend::lookup_tree( const QType &qtype, const DNSName &qname, DNSPacket *dnspkt, int zoneid )
{
  ...
  DLOG( L << Logger::Debug << d_myname << " Search = basedn: " << dn + getArg( "basedn" ) << ", filter: " << filter << ", qtype: " << qtype.getName() << endl );
...
bool LdapBackend::get( DNSResourceRecord &rr )
{
      ...
      while( d_adomain != d_adomains.end() )
      {
        while( d_attribute != d_result.end() )
        {
          ...
          while( d_value != d_attribute->second.end() )
          {
            ...
            DLOG( L << Logger::Debug << d_myname << " Record = qname: " << rr.qname << ", qtype: " << (rr.qtype).getName() << ", ttl: " << rr.ttl << ", content: " << rr.content << endl );
            return true;
          }
  ...
  {
    L << Logger::Warning << d_myname << " Search failed: " << lt.what() << endl;
    throw DBException( "LDAP server timeout" );
  }
  catch( LDAPException &le )
  {
    L << Logger::Error << d_myname << " Search failed: " << le.what() << endl;
    throw PDNSException( "LDAP server unreachable" );   // try to reconnect to another server
  }
  catch( std::exception &e )
  {
    L << Logger::Error << d_myname << " Caught STL exception for " << d_qname << ": " << e.what() << endl;
    throw DBException( "STL exception" );
  }
...
bool LdapBackend::getDomainInfo( const DNSName& domain, DomainInfo& di )
{
  ...
  try
  {
    ...
    d_pldap->getSearchEntry( msgid, result );
  }
  catch( LDAPTimeout &lt )
  {
    L << Logger::Warning << d_myname << " Unable to search LDAP directory: " << lt.what() << endl;
