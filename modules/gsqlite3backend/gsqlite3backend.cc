...
public:
  ...
  void declareArguments( const std::string & suffix = "" )
  {
    ...
    declare(suffix, "add-domain-key-query","", "insert into cryptokeys (domain_id, flags, active, publish, content) select id, :flags, :active, :publish, :content from domains where name=:domain");
    declare(suffix, "get-last-inserted-key-id-query", "", "select last_insert_rowid()");
    declare(suffix, "list-domain-keys-query","", "select cryptokeys.id, flags, active, publish, content from domains, cryptokeys where cryptokeys.domain_id=domains.id and name=:domain");
    ...
    declare(suffix, "publish-domain-key-query","", "update cryptokeys set publish=1 where domain_id=(select id from domains where name=:domain) and  cryptokeys.id=:key_id");
    declare(suffix, "unpublish-domain-key-query","", "update cryptokeys set publish=0 where domain_id=(select id from domains where name=:domain) and  cryptokeys.id=:key_id");
