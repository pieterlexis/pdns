...
void BaseLua4::prepareContext() {
  ...
  d_lw->registerFunction<boost::optional<Netmask>(NetmaskGroup::*)(const std::string&addr)>("getMatch", [](NetmaskGroup &nmg, const std::string& addr) {
      boost::optional<Netmask> ret;
      Netmask nm;
      ComboAddress ca(addr);
      if (nmg.lookup(ca, &nm)) {
        ret = nm;
      }
      return ret;
