...
bool PacketHandler::addCDS(DNSPacket& p, std::unique_ptr<DNSPacket>& r, SOAData &sd) // NOLINT(readability-identifier-length)
{
  ...
  for (const auto& value : d_dk.getEntryPoints(sd.zonename)) {
    ...
    for(auto const &digestAlgo : digestAlgos){
      rr.dr.setContent(std::make_shared<DSRecordContent>(makeDSFromDNSKey(d_slog, sd.qname(), value.first.getDNSKEY(), pdns::checked_stoi<uint8_t>(digestAlgo))));
      r->addRecord(DNSZoneRecord(rr));
      haveOne=true;
    }
  }
...
void PacketHandler::emitNSEC(std::unique_ptr<DNSPacket>& r, const DNSName& name, const DNSName& next, int mode) // NOLINT(readability-identifier-length)
{
  NSECBitmap bitmap;
  if (d_sd.qname() == name) {
...
int PacketHandler::tryAutoPrimarySynchronous(const DNSPacket& p, const DNSName& tsigkeyname)
{
  ...
  if(!haveNS) {
    SLOG(g_log << Logger::Error << "While checking for autoprimary, did not find NS for " << p.qdomain << " at: " << remote << endl,
         d_slog->info(Logr::Error, "While checking for autoprimary, did not find NS", "domain", Logging::Loggable(p.qdomain), "remote", Logging::Loggable(remote)));
    return RCode::ServFail;
  }
  ...
  if (!::arg().mustDo("allow-unsigned-autoprimary") && tsigkeyname.empty()) {
    SLOG(g_log << Logger::Error << "Received unsigned NOTIFY for " << p.qdomain << " from potential autoprimary " << remote << ". Refusing." << endl,
         d_slog->info(Logr::Error, "Received unsigned NOTIFY", "domain", Logging::Loggable(p.qdomain), "potential autoprimary", Logging::Loggable(remote)));
  ...
  if (!B.autoPrimaryBackend(remote.toString(), zonename, nsset, &nameserver, &account, &db)) {
    if (g_slogStructured) {
      ...
      d_slog->info(Logr::Error, "Unable to find backend willing to host domain", "domain", Logging::Loggable(p.qdomain), "potential autoprimary", Logging::Loggable(remote), "remote nameservers", Logging::Loggable(oss.str()));
    }
    else {
  ...
  if(pkt.d.qr) { // QR bit from dns packet (thanks RA from N)
    if(d_logDNSDetails) {
      SLOG(g_log<<Logger::Error<<"Received an answer (non-query) packet from "<<pkt.getRemoteString()<<", dropping"<<endl,
           d_slog->info(Logr::Error, "Received an answer (non-query) packet, dropping", "from", Logging::Loggable(pkt.getRemoteString())));
    }
  ...
  if(pkt.d.tc) { // truncated query. MOADNSParser would silently parse this packet in an incomplete way.
    if(d_logDNSDetails) {
      SLOG(g_log<<Logger::Error<<"Received truncated query packet from "<<pkt.getRemoteString()<<", dropping"<<endl,
           d_slog->info(Logr::Error, "Received truncated query packet, dropping", "from", Logging::Loggable(pkt.getRemoteString())));
    }
    S.inc("corrupt-packets");
    S.ringAccount("remotes-corrupt", pkt.getInnerRemote());
    return nullptr;
    ...
    if (!checkForCorrectTSIG(pkt, &tsigkeyname, &secret, &trc)) {
      ...
      if(d_logDNSDetails) {
        SLOG(g_log<<Logger::Error<<"Received a TSIG signed message with a non-validating key"<<endl,
             d_slog->info(Logr::Error, "Received a TSIG-signed message with a non-validating key"));
      }
    ...
    if(!validDNSName(pkt.qdomain)) {
      if(d_logDNSDetails) {
        SLOG(g_log<<Logger::Error<<"Received a malformed qdomain from "<<pkt.getRemoteString()<<", '"<<pkt.qdomain<<"': sending servfail"<<endl,
             d_slog->info(Logr::Error, "Received a malformed qdomain; sending servfail", "qdomain", Logging::Loggable(pkt.qdomain), "from", Logging::Loggable(pkt.getRemoteString())));
      }
      S.inc("corrupt-packets");
      S.ringAccount("remotes-corrupt", pkt.getInnerRemote());
...
std::unique_ptr<DNSPacket> PacketHandler::opcodeNotImplemented(DNSPacket& pkt, bool /* noCache */)
{
  SLOG(g_log<<Logger::Error<<"Received an unknown opcode "<<pkt.d.opcode<<" from "<<pkt.getRemoteString()<<" for "<<pkt.qdomain<<endl,
       d_slog->info(Logr::Error, "Received an unknown opcode", "opcode", Logging::Loggable(pkt.d.opcode), "from", Logging::Loggable(pkt.getRemoteString()), "query", Logging::Loggable(pkt.qdomain)));
  ...
  if (tsigTriplet.algo != g_gsstsigdnsname) {
    ...
    if (!B.getTSIGKey(*tsigkeyname, tsigTriplet.algo, secret64)) {
      SLOG(g_log << Logger::Error << "Packet for domain '" << packet.qdomain << "' denied: can't find TSIG key with name '" << *tsigkeyname << "' and algorithm '" << tsigTriplet.algo << "'" << endl,
           d_slog->info(Logr::Error, "Packet denied: can't find TSIG key with given name and algorithm", "query", Logging::Loggable(packet.qdomain), "key", Logging::Loggable(*tsigkeyname), "algorithm", Logging::Loggable(tsigTriplet.algo)));
      return false;
    }
