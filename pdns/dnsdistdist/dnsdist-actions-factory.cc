...
public:
  ...
  DNSResponseAction::Action operator()(DNSResponse* response, std::string* ruleresult) const override
  {
    ...
    if (d_delay){
      response->ids.delayedResponseMsgs.push_back(std::make_pair(std::unique_ptr<DNSDistProtoBufMessage>(std::make_unique<DNSDistProtoBufMessage>(message)), std::shared_ptr<RemoteLoggerInterface>(d_logger))) ;
    }
    else {
      static thread_local std::string data;
      data.clear();
