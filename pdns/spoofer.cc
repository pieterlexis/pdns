...
{
  if (argc != 4) {
    cerr<<"Usage :"<<argv[0]<<" <destination> <source prefix (/16, e.g. 192.168) to spoof> <number of addresses to spoof>"<<endl;
    return EXIT_FAILURE;
  }

  ComboAddress dest(argv[1]);
  std::string sourceNetwork(argv[2]);
  unsigned long nbAddresses = std::stoul(argv[3]);
  ...
  unsigned long idx = 0;
  for (;;) {
    makeQuery(packet, DNSName("target.powerdns.com."), QType::A);
